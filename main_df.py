...
### this section for splitting training data before use
n = 10000
...
train_gen = train_datagen.flow_from_dataframe(dataframe=train_bal_df, directory='/media/steve/Samsung_T5/MIMICCXR',
...
val_gen = val_datagen.flow_from_dataframe(dataframe=val_bal_df, directory='/media/steve/Samsung_T5/MIMICCXR',
                                          x_col='path', y_col=finding, target_size=(299, 299), color_mode='rgb',
                                          class_mode='categorical', batch_size=16, shuffle=True, classes=class_list)
...
# build the finetune model with initialized bias for detecting minority class.
# If this section is included, remove next section, and should test with imbalanced data.
# TODO: try model with  focal loss in phase II
# TODO: try model as single stage with focal loss and pre-initialized probabilities for top layer
"""
...
# should try just flattening and then adding the top layer
...
logger.error('Phase I eval on balanced set: ' + str(e_phaseI))
...
for layer in model.layers[133:]:
    layer.trainable = True
# TODO: compute weight based on training set proportions
class_weights = {0: 1.,
                 1: 5.}
...
# directory='/home/steve/PycharmProjects/mimic-cxr/data',
# directory='/media/steve/Samsung_T5/MIMICCXR',
phaseII_train_gen = train_datagen.flow_from_dataframe(dataframe=train_df, directory='/media/steve/Samsung_T5/MIMICCXR',
                                                      x_col='path', y_col=finding, target_size=(299, 299),
                                                      color_mode='rgb', class_mode='categorical', batch_size=16,
                                                      shuffle=True, classes=class_list)
phaseII_val_gen = val_datagen.flow_from_dataframe(dataframe=val_df, directory='/media/steve/Samsung_T5/MIMICCXR',
                                                  x_col='path', y_col=finding, target_size=(299, 299), color_mode='rgb',
...
e_phaseII = finetune_model.evaluate_generator(phaseII_val_gen, steps=STEP_SIZE_VALID, verbose=1)
logger.error('Phase I eval on imbalanced dataset: ' + str(e_phaseII))
...
logger.error('Phase II eval on imbalanced dataset: ' + str(e))
